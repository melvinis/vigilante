...
async function persistScan(scan,user,prev){
  ...
  try{
    ...
    await sb.upsert("wallet_registry",{address:scan.address.toLowerCase(),chain:scan.chain,entity:scan.entity,latest_score:scan.overallScore,latest_decision:scan.decision,last_scan:new Date().toISOString(),trend:sd,status_flag:sf,balance:scan.balance,scan_count:(prev?.scan_count||0)+1,...(!prev?{first_scan:new Date().toISOString()}:{})});
