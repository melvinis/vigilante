...
class SupabaseClient {
  ...
  headers(extra = {}) {
    return { "Content-Type":"application/json", "apikey":this.anonKey, "Authorization":`Bearer ${this.token||this.anonKey}`, ...extra };
  }
  async signIn(email, password) {
    const res = await fetch(`${this.url}/auth/v1/token?grant_type=password`, { method:"POST", headers:{"Content-Type":"application/json","apikey":this.anonKey}, body:JSON.stringify({email,password}) });
    const d = await res.json();
    if (!res.ok) throw new Error(d.error_description||d.msg||"Login failed");
    this.token = d.access_token;
    return { user:d.user, token:d.access_token, refreshToken:d.refresh_token };
  }
  async signOut() {
  ...
  async setPasswordWithToken(accessToken, newPassword) {
    const res = await fetch(`${this.url}/auth/v1/user`, {
      method: "PUT",
      headers: { "Content-Type":"application/json", "apikey":this.anonKey, "Authorization":`Bearer ${accessToken}` },
      body: JSON.stringify({ password: newPassword }),
    });
    const d = await res.json();
    if (!res.ok) throw new Error(d.message||d.msg||"Password update failed");
    return d;
  }
  async refreshToken(refreshToken) {
    const res = await fetch(`${this.url}/auth/v1/token?grant_type=refresh_token`, { method:"POST", headers:{"Content-Type":"application/json","apikey":this.anonKey}, body:JSON.stringify({refresh_token:refreshToken}) });
    const d = await res.json();
  ...
  async select(table, params={}) {
    ...
    const res = await fetch(`${this.url}/rest/v1/${table}?${q}`, { headers:this.headers({"Prefer":"return=representation"}) });
    if (!res.ok) { const e=await res.json(); throw new Error(e.message||`Select failed: ${table}`); }
    return res.json();
  }
  async upsert(table, data) {
    const res = await fetch(`${this.url}/rest/v1/${table}`, { method:"POST", headers:this.headers({"Prefer":"resolution=merge-duplicates,return=representation"}), body:JSON.stringify(data) });
    if (!res.ok) { const e=await res.json(); throw new Error(e.message||`Upsert failed: ${table}`); }
    return res.json();
  }
  async insert(table, data) {
    const res = await fetch(`${this.url}/rest/v1/${table}`, { method:"POST", headers:this.headers({"Prefer":"return=representation"}), body:JSON.stringify(data) });
    if (!res.ok) { const e=await res.json(); throw new Error(e.message||`Insert failed: ${table}`); }
    return res.json();
  }
  isConfigured() { return !!(this.url && this.anonKey && this.url.includes("supabase.co")); }
}

// Singleton — initialised from env vars (set in Vercel dashboard)
const VITE_SB_URL  = import.meta.env.VITE_SUPABASE_URL  || "";
const VITE_SB_KEY  = import.meta.env.VITE_SUPABASE_ANON_KEY || "";
let sb = new SupabaseClient(VITE_SB_URL, VITE_SB_KEY);

// ═══════════════════════════════════════════════════════════════════════════
// LOCAL STORAGE HELPERS  (replaces window.storage from artifact version)
// ═══════════════════════════════════════════════════════════════════════════
const ls = {
...
const DEFI_REGISTRY = {
  ...
  "0x1116898dda4015ed8ddefb84b6e8bc24528af2d8":{label:"Harmony Horizon Bridge",risk:90,cat:"HACKED_BRIDGE",protocol:"Harmony"},
  ...
  "0x68b3465833fb72a70ecdf485e0e4c7bd8665fc45":{label:"Uniswap Universal Router",risk:8,cat:"DEFI",protocol:"Uniswap"},
  ...
  "0xc02aaa39b223fe8d0a0e5c4f27ead9083c756cc2":{label:"WETH",risk:3,cat:"STABLECOIN",protocol:"Ethereum"},
...
const EXCHANGE_WHITELIST = new Set(["0x3f5ce5fbfe3e9af3971dd833d26ba9b5c936f0be","0xd551234ae421e3bcba99a0da6d736074f22192ff","0xa9d1e08c7793af67e9d92fe308d5697fb81d3e43","0x71660c4005ba85c37ccec55d0c4493e66fe775d3","1NDyJtNTjmwk5xPNhjgAMu4HDHigtobu1s","TJDENsfBJs4RFETt1X1W8wMDc8M5XnJhCe"]);
const OFAC_LIVE = { addresses:{}, lastFetch:0, count:0, status:"idle" };
const HOP_DECAY = [1.0,0.40,0.16,0.064,0.026,0.010];

function lookupAll(addr) {
  if(!addr)return null;
  const a=addr.toLowerCase?.();
  return OFAC_LIVE.addresses[a]||OFAC_LIVE.addresses[addr]||SANCTIONS_DB[a]||SANCTIONS_DB[addr]||DEFI_REGISTRY[a]||DEFI_REGISTRY[addr]||EXCHANGE_LABELS[a]||EXCHANGE_LABELS[addr]||null;
}
function isWL(addr){return EXCHANGE_WHITELIST.has(addr?.toLowerCase())||EXCHANGE_WHITELIST.has(addr);}
...
// OFAC FEED
// ═══════════════════════════════════════════════════════════════════════════
// Three proxy fallbacks, 45s timeout, smaller SDN text file (~200KB vs 20MB XML)
...
function _parseBTC(addr,ad,txs){
  const coSpend=new Set();const cps=[];const changeAddr=[];
  for(const tx of txs.slice(0,15)){
    if(tx.vin?.length>1)tx.vin.forEach(i=>i.prevout?.scriptpubkey_address&&coSpend.add(i.prevout.scriptpubkey_address));
    const changeOuts=(tx.vout||[]).filter(o=>o.value&&o.value%1000000!==0);
    changeOuts.forEach(o=>{if(o.scriptpubkey_address&&o.scriptpubkey_address!==addr)changeAddr.push(o.scriptpubkey_address);});
    (tx.vout||[]).forEach(o=>{if(o.scriptpubkey_address&&o.scriptpubkey_address!==addr){const k=lookupAll(o.scriptpubkey_address);cps.push({address:o.scriptpubkey_address,amount:(o.value/1e8).toFixed(8)+" BTC",label:k?.label||"Unknown",cat:k?.cat||"UNKNOWN",knownRisk:k?.risk??null,direction:"OUT",hop:0});}});
    tx.vin?.forEach(i=>{const a=i.prevout?.scriptpubkey_address;if(a&&a!==addr){const k=lookupAll(a);cps.push({address:a,amount:((i.prevout?.value||0)/1e8).toFixed(8)+" BTC",label:k?.label||"Unknown",cat:k?.cat||"UNKNOWN",knownRisk:k?.risk??null,direction:"IN",hop:0});}});
  }
  const first=txs[txs.length-1];
  return{balance:((ad.chain_stats?.funded_txo_sum-ad.chain_stats?.spent_txo_sum||0)/1e8).toFixed(8)+" BTC",txCount:ad.chain_stats?.tx_count||0,firstSeen:first?.status?.block_time?new Date(first.status.block_time*1000).toISOString().slice(0,10):"Unknown",counterparties:cps,coSpendCount:coSpend.size,tokenActivity:[],changeAddresses:[...new Set(changeAddr)]};
}
async function _ethES(addr,key){
  ...
  const fundingWallet=[...txs].sort((a,b)=>parseInt(a.timeStamp)-parseInt(b.timeStamp)).find(t=>t.to?.toLowerCase()===addr.toLowerCase())?.from||null;
  const nonces=txs.map(t=>parseInt(t.nonce)).sort((a,b)=>a-b);
  const nonceGaps=[];for(let i=1;i<nonces.length;i++){if(nonces[i]-nonces[i-1]>50)nonceGaps.push({from:nonces[i-1],to:nonces[i]});}
  const seen=new Set();
  const cps=txs.slice(0,20).map(tx=>{const peer=tx.from?.toLowerCase()===addr.toLowerCase()?tx.to:tx.from;if(!peer||seen.has(peer.toLowerCase()))return null;seen.add(peer.toLowerCase());const k=lookupAll(peer);return{address:peer,amount:(parseInt(tx.value||0)/1e18).toFixed(6)+" ETH",label:k?.label||(tx.input!=="0x"?"Contract":"Unknown"),cat:k?.cat||(tx.input!=="0x"?"CONTRACT":"UNKNOWN"),knownRisk:k?.risk??null,direction:tx.from?.toLowerCase()===addr.toLowerCase()?"OUT":"IN",hop:0,protocol:k?.protocol||null};}).filter(Boolean);
  const tokenActivity=Object.values(toks.reduce((a,t)=>{if(!a[t.tokenSymbol])a[t.tokenSymbol]={symbol:t.tokenSymbol,name:t.tokenName,count:0};a[t.tokenSymbol].count++;return a},{}));
  let contractLabel=null;
  try{const cr=await fetch(`${b}?module=contract&action=getsourcecode&address=${addr}&apikey=${k}`,{signal:AbortSignal.timeout(5000)});const cd=await cr.json();if(cd.status==="1"&&cd.result?.[0]?.ContractName)contractLabel=cd.result[0].ContractName;}catch(e){}
  const first=txs[txs.length-1];
  return{balance:bd.result?(parseInt(bd.result)/1e18).toFixed(6)+" ETH":"N/A",txCount:txs.length,firstSeen:first?new Date(parseInt(first.timeStamp)*1000).toISOString().slice(0,10):"Unknown",counterparties:cps,tokenActivity,coSpendCount:0,changeAddresses:[],fundingWallet,nonceGaps,contractLabel};
}
async function _ethBC(addr){
  ...
  const cps=(txd.items||[]).slice(0,15).map(tx=>{const peer=tx.from?.hash?.toLowerCase()===addr.toLowerCase()?tx.to?.hash:tx.from?.hash;const k=lookupAll(peer);return{address:peer,amount:(parseInt(tx.value||0)/1e18).toFixed(6)+" ETH",label:k?.label||"Unknown",cat:k?.cat||"UNKNOWN",knownRisk:k?.risk??null,direction:tx.from?.hash?.toLowerCase()===addr.toLowerCase()?"OUT":"IN",hop:0};}).filter(c=>c.address);
...
async function _trxTS(addr){
  ...
  const cps=txs.slice(0,20).map(tx=>{const peer=tx.ownerAddress===addr?tx.toAddress:tx.ownerAddress;if(!peer||seen.has(peer))return null;seen.add(peer);const k=lookupAll(peer);return{address:peer,amount:((tx.contractData?.amount||0)/1e6).toFixed(4)+" TRX",label:k?.label||"Unknown",cat:k?.cat||"UNKNOWN",knownRisk:k?.risk??null,direction:tx.ownerAddress===addr?"OUT":"IN",hop:0};}).filter(Boolean);
  const first=txs[txs.length-1];
...
async function _trxTG(addr){
  const res=await fetch(`https://api.trongrid.io/v1/accounts/${addr}/transactions?limit=20`,{signal:AbortSignal.timeout(8000)});
  if(!res.ok)throw new Error(`HTTP ${res.status}`);
  const d=await res.json();const txs=d.data||[];
  const cps=txs.slice(0,15).map(tx=>{const c=tx.raw_data?.contract?.[0];const owner=c?.parameter?.value?.owner_address,to=c?.parameter?.value?.to_address;const peer=owner===addr?to:owner;const k=lookupAll(peer);return{address:peer,amount:((c?.parameter?.value?.amount||0)/1e6).toFixed(4)+" TRX",label:k?.label||"Unknown",cat:k?.cat||"UNKNOWN",knownRisk:k?.risk??null,direction:owner===addr?"OUT":"IN",hop:0};}).filter(c=>c.address);
...
async function multiHop(rootAddr,chain,rootCps,maxHops,key,push,onUpdate){
  const graph=new Map();const visited=new Set([rootAddr.toLowerCase()]);const queue=[];
  for(const cp of rootCps.slice(0,8)){if(!cp.address||visited.has(cp.address.toLowerCase()))continue;visited.add(cp.address.toLowerCase());const node={...cp,hop:1,propagatedRisk:(cp.knownRisk||0)*HOP_DECAY[1]};graph.set(cp.address,node);if((cp.knownRisk!==null&&cp.knownRisk>30)||cp.cat==="UNKNOWN"||cp.cat==="MIXER")queue.push({address:cp.address,hop:1});}
  for(let hop=2;hop<=maxHops&&queue.length>0;hop++){
    const level=queue.splice(0,Math.min(queue.length,3));push&&push(`Hop ${hop}/${maxHops}: scanning ${level.length} nodes…`);
    for(const{address,hop:ph}of level){if(ph>=maxHops)continue;try{await new Promise(r=>setTimeout(r,350));let hd;if(chain==="BTC")hd=await _btcBS(address).catch(()=>null);else if(chain==="ETH")hd=await _ethES(address,key).catch(()=>null);else if(chain==="TRX")hd=await _trxTS(address).catch(()=>null);if(!hd)continue;for(const cp of(hd.counterparties||[]).slice(0,5)){if(!cp.address||visited.has(cp.address.toLowerCase()))continue;visited.add(cp.address.toLowerCase());const decay=HOP_DECAY[hop]||0.01;const node={...cp,hop,propagatedRisk:(cp.knownRisk||0)*decay,parentAddress:address};graph.set(cp.address,node);if(node.propagatedRisk>3||cp.cat==="MIXER"||cp.cat==="SANCTIONS")queue.push({address:cp.address,hop});}}catch(e){}}
    onUpdate&&onUpdate(hop,[...graph.values()]);
  }
  return[...graph.values()];
}

function scoreWallet(addr,chain,data,hopNodes){
  const direct=lookupAll(addr);
  ...
  for(const cp of all){if(cp.knownRisk===null||cp.knownRisk===undefined)continue;const d=HOP_DECAY[cp.hop||0]||0.01;const p=cp.knownRisk*d;if(cp.cat==="SANCTIONS")sig.sanctionsExposure=Math.max(sig.sanctionsExposure,p);else if(cp.cat==="DARKNET")sig.darknetExposure=Math.max(sig.darknetExposure,p);else if(cp.cat==="MIXER")sig.mixerTumblerUsage=Math.max(sig.mixerTumblerUsage,p);else if(cp.cat==="RANSOMWARE")sig.ransomwareExposure=Math.max(sig.ransomwareExposure,p);else if(cp.cat==="HACKED_BRIDGE")sig.stolenFundsRisk=Math.max(sig.stolenFundsRisk,p);else if(cp.cat==="EXCHANGE")sig.exchangeRisk=Math.max(sig.exchangeRisk,cp.knownRisk*d*0.1);}
  if(chain==="BTC"&&coSpendCount>10)sig.exchangeRisk=Math.min(40,coSpendCount*1.5);
  for(const ca of changeAddresses.slice(0,5)){const k=lookupAll(ca);if(k&&k.risk>50)sig.mixerTumblerUsage=Math.max(sig.mixerTumblerUsage,k.risk*0.3);}
  if(fundingWallet){const k=lookupAll(fundingWallet);if(k&&k.risk>50)sig.sanctionsExposure=Math.max(sig.sanctionsExposure,k.risk*0.35);}
  ...
  const domCat=counterparties.reduce((a,c)=>{a[c.cat]=(a[c.cat]||0)+1;return a},{});
  if((domCat.EXCHANGE||0)>counterparties.length*0.5)sig.peerToPeerExposure=0;
  const unkR=counterparties.filter(c=>c.cat==="UNKNOWN").length/Math.max(counterparties.length,1);
  if(unkR>0.8&&counterparties.length>3)sig.peerToPeerExposure=Math.max(sig.peerToPeerExposure,18);
  ...
  const topCat=Object.entries(domCat).sort((a,b)=>b[1]-a[1])[0]?.[0]||"UNKNOWN";
  ...
  return{overallScore,signals:sig,entity:contractLabel||eMap[topCat]||"Unhosted Wallet",directMatch:false,entitySource:contractLabel?"CONTRACT_LABEL":"HEURISTIC"};
}
...
function analyzeDiff(older,newer){
  ...
  for(const k of Object.keys(newer.signals||{})){const oldV=Math.round(older.signals?.[k]||0),newV=Math.round(newer.signals?.[k]||0),d=newV-oldV;if(Math.abs(d)>=3)changes.push({type:d>0?"SIGNAL_UP":"SIGNAL_DOWN",field:k.replace(/([A-Z])/g," $1").trim(),from:oldV,to:newV,delta:d,severity:Math.abs(d)>=20?"HIGH":Math.abs(d)>=8?"MEDIUM":"LOW"});}
  ...
  const oldAddrs=new Set((older.counterparties||[]).map(c=>c.address?.toLowerCase()));
  for(const cp of(newer.counterparties||[]).filter(c=>!oldAddrs.has(c.address?.toLowerCase())&&c.knownRisk!==null&&c.knownRisk>40)){changes.push({type:"NEW_COUNTERPARTY",field:"New High-Risk Counterparty",from:null,to:`${cp.label} (${cp.address?.slice(0,10)}…)`,delta:cp.knownRisk,severity:cp.knownRisk>=80?"HIGH":"MEDIUM"});}
  let statusFlag="STABLE";
  if(scoreDelta>=10)statusFlag="ESCALATED";else if(scoreDelta<=-10)statusFlag="IMPROVED";else if(changes.some(c=>c.type==="DECISION_CHANGE"))statusFlag="STATUS_CHANGE";else if(changes.some(c=>c.severity==="HIGH"))statusFlag="ALERT";
  return{changes,scoreDelta,statusFlag};
}

// ═══════════════════════════════════════════════════════════════════════════
// SUPABASE PERSISTENCE
// ═══════════════════════════════════════════════════════════════════════════
async function persistScan(scanResult,user,prevScan){
  if(!sb.isConfigured()||!sb.token)return{ok:false,reason:"not_configured"};
  try{
    const diff=prevScan?analyzeDiff({...prevScan,overallScore:prevScan.overall_score,decision:prevScan.decision,entity:prevScan.entity,balance:prevScan.balance,counterparties:prevScan.counterparties||[]},scanResult):null;
    const scoreDelta=diff?.scoreDelta??0;const statusFlag=diff?.statusFlag??"NEW";
    await sb.upsert("wallet_registry",{address:scanResult.address.toLowerCase(),chain:scanResult.chain,entity:scanResult.entity,latest_score:scanResult.overallScore,latest_decision:scanResult.decision,last_scan:new Date().toISOString(),trend:scoreDelta,status_flag:statusFlag,balance:scanResult.balance,...(!prevScan?{first_scan:new Date().toISOString(),scan_count:1}:{})});
    const inserted=await sb.insert("wallet_scans",{address:scanResult.address.toLowerCase(),chain:scanResult.chain,scanned_at:new Date().toISOString(),overall_score:scanResult.overallScore,risk_level:scanResult.overallScore<=25?"LOW":scanResult.overallScore<=54?"MEDIUM":scanResult.overallScore<=74?"HIGH":"CRITICAL",decision:scanResult.decision,entity:scanResult.entity,entity_source:scanResult.entitySource,balance:scanResult.balance,tx_count:scanResult.txCount||0,first_seen:scanResult.firstSeen,signals:scanResult.signals,counterparties:(scanResult.counterparties||[]).slice(0,30),token_activity:scanResult.tokenActivity||[],hop_node_count:scanResult.hopNodeCount||0,direct_match:scanResult.directMatch||false,funding_wallet:scanResult.fundingWallet||null,api_source:scanResult._src||null,analyst_note:scanResult.note||null,status_flag:statusFlag,score_delta:scoreDelta,scanned_by:user?.id||null,scanned_by_email:user?.email||null});
    const scanId=inserted?.[0]?.id;
    if(diff?.changes?.length&&scanId){await sb.insert("scan_changes",diff.changes.map(c=>({address:scanResult.address.toLowerCase(),scan_id:scanId,change_type:c.type,field:c.field,from_value:c.from!==null?String(c.from):null,to_value:c.to!==null?String(c.to):null,delta:c.delta,severity:c.severity}))).catch(()=>{});}
    return{ok:true,scanId};
  }catch(e){return{ok:false,reason:e.message};}
}

// ═══════════════════════════════════════════════════════════════════════════
// RISK HELPERS & UI COMPONENTS
// ═══════════════════════════════════════════════════════════════════════════
...
const catColor={SANCTIONS:"#FF2D55",DARKNET:"#FF2D55",MIXER:"#FF8500",RANSOMWARE:"#FF2D55",HACKED_BRIDGE:"#FF8500",EXCHANGE:"#00D98A",DEFI:"#00B8E8",STABLECOIN:"#00E5CC",CONTRACT:"#9B6DFF",WALLET:"#567090",PROGRAM:"#2E4060",UNKNOWN:"#567090"};
...
export default function App() {
  ...
  // Password-set flow (from invite email redirect)
  const [inviteToken,setInviteToken]=useState(null);
  const [newPwd,setNewPwd]=useState("");
  const [newPwdConfirm,setNewPwdConfirm]=useState("");
  const [pwdSetLoading,setPwdSetLoading]=useState(false);
  const [pwdSetErr,setPwdSetErr]=useState("");
  const [pwdSetDone,setPwdSetDone]=useState(false);
  ...
  // ── On mount: handle Supabase invite/reset redirect, restore session ──
  useEffect(()=>{
    (async()=>{
      // Parse hash tokens from Supabase email redirect
      ...
      if(sess){
        try{
          const refreshed=await sb.refreshToken(sess.refreshToken);
          sb.token=refreshed.access_token;
          setUser(sess.user);
          ls.set("sbSession",{...sess,token:refreshed.access_token});
        }catch(e){ls.del("sbSession");}
      }
      // OFAC
      setOfacStatus("loading");
      await fetchOFAC(push);
      setOfacStatus(OFAC_LIVE.status);
    })();
  },[]);

  useEffect(()=>{if(user)refreshRegistry();},[user]);
  ...
  const refreshRegistry=async()=>{
    ...
    try{const reg=await sb.select("wallet_registry",{order:"last_scan.desc",limit:200});setRegistry(reg||[]);}
    catch(e){}finally{setRegistryLoading(false);}
  };
  ...
  const handleLogin=async()=>{
    ...
    try{
      ...
      setUser(u);ls.set("sbSession",{user:u,token,refreshToken});
  ...
  const handleLogout=async()=>{
    await sb.signOut();setUser(null);setRegistry([]);ls.del("sbSession");
  };

  const handleSetPassword=async()=>{
    if(newPwd!==newPwdConfirm){setPwdSetErr("Passwords do not match");return;}
    if(newPwd.length<8){setPwdSetErr("Password must be at least 8 characters");return;}
    setPwdSetLoading(true);setPwdSetErr("");
    try{
      await sb.setPasswordWithToken(inviteToken,newPwd);
      setPwdSetDone(true);
      setTimeout(()=>{setInviteToken(null);setPwdSetDone(false);},2000);
    }catch(e){setPwdSetErr(e.message);}
    setPwdSetLoading(false);
  };

  const openHistory=async(address)=>{
    setSelectedWallet(address);setTab("history");setHistoryTab("timeline");
    try{
      const[scans,changes]=await Promise.all([sb.select("wallet_scans",{filter:{address:address.toLowerCase()},order:"scanned_at.desc",limit:100}),sb.select("scan_changes",{filter:{address:address.toLowerCase()},order:"created_at.desc",limit:200})]);
      setWalletScans(scans||[]);setWalletChanges(changes||[]);
  ...
  const screen=useCallback(async()=>{
    ...
    if(chain==="UNKNOWN"){setErr("Unrecognised address. Supported: BTC, ETH/EVM, TRX, SOL");return;}
    setLoading(true);setErr(null);setResult(null);setNote("");setHopNodes([]);stRef.current=[];setStatus([]);
    try{
      push(`Chain: ${chain}`);
      let data;
      if(chain==="BTC"){push("Fetching BTC…");data=await fetchBTC(a,push);}
      else if(chain==="ETH"){push("Fetching ETH…");data=await fetchETH(a,keys.etherscan,push);}
      else if(chain==="TRX"){push("Fetching TRX…");data=await fetchTRX(a,push);}
      else{push("Fetching SOL…");data=await fetchSOL(a,push);}
      push(`${data.txCount} txs · ${data.balance}`);
      push(`${maxHops}-hop traversal…`);
      const hops=await multiHop(a,chain,data.counterparties,maxHops,keys.etherscan,push,(h,n)=>setHopNodes([...n]));
      setHopNodes(hops);
      const score=scoreWallet(a,chain,data,hops);
      const decision=score.overallScore<=policy.accept?"ACCEPT":score.overallScore<=policy.review?"REVIEW":"REJECT";
      const ts=new Date().toISOString().replace("T"," ").slice(0,19);
      const full={address:a,chain,...data,...score,decision,ts,hopNodeCount:hops.length,note:""};
      setResult(full);
      if(sb.isConfigured()&&sb.token){
        push("Saving to Supabase…");setSyncStatus("syncing");
        let prevScan=null;
        try{const prev=await sb.select("wallet_scans",{filter:{address:a.toLowerCase()},order:"scanned_at.desc",limit:1});prevScan=prev?.[0]||null;}catch(e){}
        const p=await persistScan(full,user,prevScan);
        if(p.ok){push(`✓ Saved (ID: ${p.scanId?.slice(0,8)}…)`);setSyncStatus("ok");}
        else{push(`⚠ Save failed: ${p.reason}`);setSyncStatus("error");}
        await refreshRegistry();
        if(selectedWallet?.toLowerCase()===a.toLowerCase())await openHistory(a);
      }else{push("⚠ Not synced — not logged in");}
    }catch(e){setErr(e.message);setSyncStatus("error");}
    setLoading(false);
  },[addr,keys,policy,maxHops,push,user,selectedWallet]);

  const exportCSV=async()=>{
    const rows=[["Timestamp","Address","Chain","Score","Risk","Entity","Decision","Balance","TX Count","Scanned By","Score Delta"]];
    ...
    allScans.flat().sort((a,b)=>new Date(b.scanned_at)-new Date(a.scanned_at)).forEach(e=>rows.push([e.scanned_at,e.address,e.chain,e.overall_score,e.risk_level,e.entity,e.decision,e.balance,e.tx_count,e.scanned_by_email||"",e.score_delta||0]));
    const el=document.createElement("a");el.href=URL.createObjectURL(new Blob([rows.map(r=>r.join(",")).join("\n")],{type:"text/csv"}));el.download=`asas_aml_${Date.now()}.csv`;el.click();
  };

  const decCol=result?(result.decision==="ACCEPT"?T.green:result.decision==="REVIEW"?T.yellow:T.red):T.mid;

  // ── SET PASSWORD SCREEN (invite redirect) ──
  if(inviteToken)return(
    <div style={{minHeight:"100vh",background:T.bg,display:"flex",alignItems:"center",justifyContent:"center",fontFamily:T.mono,padding:24}}>
      <div style={{background:T.panel,border:`1px solid ${T.borderHi}`,borderRadius:10,padding:32,width:"100%",maxWidth:400}}>
        ...
        {pwdSetDone?<div style={{padding:"16px",background:`${T.green}15`,border:`1px solid ${T.green}40`,borderRadius:6,color:T.green,fontSize:11,textAlign:"center"}}>✓ Password set successfully! Redirecting to login…</div>:<>
          ...
          {pwdSetErr&&<div style={{marginBottom:12,fontSize:10,color:T.red,padding:"7px 10px",background:`${T.red}10`,border:`1px solid ${T.red}30`,borderRadius:4}}>{pwdSetErr}</div>}
          <button onClick={handleSetPassword} disabled={pwdSetLoading||!newPwd||!newPwdConfirm} style={{width:"100%",padding:"11px",borderRadius:5,background:pwdSetLoading?T.border:`linear-gradient(135deg,${T.accent},${T.accentLo})`,border:"none",color:T.bg,fontFamily:T.mono,fontSize:11,fontWeight:700,letterSpacing:"0.12em",cursor:pwdSetLoading?"not-allowed":"pointer"}}>
            {pwdSetLoading?"SETTING PASSWORD…":"SET PASSWORD & CONTINUE"}
          </button>
  ...
  if(!user)return(
    <div style={{minHeight:"100vh",background:T.bg,display:"flex",alignItems:"center",justifyContent:"center",fontFamily:T.mono,padding:24}}>
      <div style={{background:T.panel,border:`1px solid ${T.borderHi}`,borderRadius:10,padding:32,width:"100%",maxWidth:400}}>
        ...
        <button onClick={handleLogin} disabled={loginLoading||!loginEmail||!loginPwd} style={{width:"100%",padding:"12px",borderRadius:5,background:loginLoading?T.border:`linear-gradient(135deg,${T.accent},${T.accentLo})`,border:"none",color:loginLoading?T.dim:T.bg,fontFamily:T.mono,fontSize:12,fontWeight:700,letterSpacing:"0.12em",cursor:loginLoading||!loginEmail||!loginPwd?"not-allowed":"pointer",marginBottom:16}}>
          {loginLoading?"SIGNING IN…":"SIGN IN"}
        </button>
        <div style={{padding:"10px 12px",background:`${T.accent}08`,border:`1px solid ${T.accent}20`,borderRadius:4,fontSize:8.5,color:T.dim,lineHeight:1.7}}>
          Team members receive an invite email from Supabase. Click the link in that email — it will now open this page and prompt you to set your password directly here.
        </div>
      </div>
    </div>
  );
    ...
    <div style={{minHeight:"100vh",background:T.bg,color:T.text,fontFamily:T.mono}}>
      ...
      <div style={{borderBottom:`1px solid ${T.border}`,padding:"11px 24px",display:"flex",alignItems:"center",justifyContent:"space-between",background:`${T.surface}EE`,backdropFilter:"blur(16px)",position:"sticky",top:0,zIndex:20,flexWrap:"wrap",gap:8}}>
        ...
        <div style={{display:"flex",alignItems:"center",gap:10,flexWrap:"wrap"}}>
          ...
          <div style={{display:"flex",alignItems:"center",gap:5}}><Dot c={ofacStatus==="ok"?T.green:T.yellow} s={5}/><span style={{fontSize:8,color:T.mid}}>OFAC {ofacStatus==="ok"?OFAC_LIVE.count:ofacStatus}</span></div>
      ...
      {showKeys&&<div style={{background:T.surface,borderBottom:`1px solid ${T.border}`,padding:"12px 24px"}}>
        <div style={{fontSize:8.5,color:T.mid,marginBottom:5}}>Etherscan API Key <span style={{color:T.dim}}>— free at etherscan.io/apis · improves ETH rate limits</span></div>
      ...
      <div style={{padding:"0 24px",borderBottom:`1px solid ${T.border}`,display:"flex",overflowX:"auto"}}>
        {[["screen","SCREEN"],["wallets","REGISTRY"],["history","HISTORY"],["log","AUDIT LOG"]].map(([id,label])=>(
      ...
      <div style={{padding:24}}>
        ...
        {tab==="screen"&&<>
          <div style={{display:"flex",gap:10,marginBottom:18,maxWidth:820}}>
            <div style={{flex:1,position:"relative"}}>
              <input value={addr} onChange={e=>setAddr(e.target.value)} onKeyDown={e=>e.key==="Enter"&&screen()} placeholder="Wallet address — BTC · ETH/EVM · TRX · SOL" style={{width:"100%",background:T.panel,border:`1px solid ${T.borderHi}`,borderRadius:6,padding:"12px 16px",color:T.text,fontSize:12,fontFamily:T.mono,outline:"none",boxSizing:"border-box"}} onFocus={e=>e.target.style.borderColor=T.accent} onBlur={e=>e.target.style.borderColor=T.borderHi}/>
              {addr&&<span style={{position:"absolute",right:12,top:"50%",transform:"translateY(-50%)",fontSize:8.5,color:T.accentLo,pointerEvents:"none"}}>{detectChain(addr.trim())}</span>}
            </div>
          ...
          {result&&!loading&&(()=>{
            const prevScan=walletScans[0];
            const diff=prevScan?analyzeDiff({...prevScan,overallScore:prevScan.overall_score,decision:prevScan.decision,entity:prevScan.entity,balance:prevScan.balance,counterparties:prevScan.counterparties||[]},result):null;
            return <div style={{display:"grid",gridTemplateColumns:"minmax(0,270px) 1fr",gap:16,maxWidth:1120}}>
              <div style={{display:"flex",flexDirection:"column",gap:14}}>
                ...
                <div style={{background:T.panel,border:`1px solid ${T.border}`,borderRadius:8,padding:16}}>
                  ...
                  <button onClick={()=>openHistory(result.address)} style={{marginTop:12,width:"100%",padding:"6px",borderRadius:4,background:`${T.purple}15`,border:`1px solid ${T.purple}40`,color:T.purple,fontSize:9,fontFamily:T.mono,cursor:"pointer",letterSpacing:"0.1em"}}>VIEW FULL HISTORY →</button>
                </div>
              </div>
              <div style={{display:"flex",flexDirection:"column",gap:14}}>
                <div style={{background:T.panel,border:`1px solid ${T.border}`,borderRadius:8,padding:16}}>
                  <div style={{fontSize:7.5,color:T.dim,letterSpacing:"0.2em",marginBottom:14}}>RISK SIGNALS</div>
                  <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:"0 24px"}}>{Object.entries(result.signals||{}).map(([k,v])=><Bar key={k} label={k} value={v} prevValue={prevScan?.signals?.[k]}/>)}</div>
                </div>
                {result.counterparties?.length>0&&<div style={{background:T.panel,border:`1px solid ${T.border}`,borderRadius:8,overflow:"hidden"}}>
                  ...
                  <div style={{maxHeight:190,overflow:"auto"}}>{[...result.counterparties,...hopNodes].slice(0,20).map((cp,i)=>(
                    <div key={i} style={{display:"flex",alignItems:"center",gap:8,padding:"6px 12px",borderBottom:`1px solid ${T.border}10`,background:i%2?`${T.panel}50`:"transparent"}}>
                      ...
                      <span style={{fontSize:8.5,color:catColor[cp.cat]||T.mid,flex:1}}>{cp.cat}</span>
                ...
                <div style={{background:T.panel,border:`1px solid ${T.border}`,borderRadius:8,padding:16}}>
                  ...
                  <textarea value={note} onChange={e=>setNote(e.target.value)} placeholder="EDD rationale, override justification, compliance notes…" style={{width:"100%",background:T.bg,border:`1px solid ${T.border}`,borderRadius:4,padding:"8px 12px",color:T.text,fontSize:10,fontFamily:T.mono,resize:"vertical",minHeight:52,outline:"none",boxSizing:"border-box",lineHeight:1.6}} onFocus={e=>e.target.style.borderColor=T.accent} onBlur={e=>e.target.style.borderColor=T.border}/>
                </div>
              </div>
            </div>;
          })()}
          ...
          {!result&&!loading&&!err&&<div style={{textAlign:"center",padding:"60px 0",color:T.dim}}>
            ...
            <div style={{fontSize:8.5,opacity:0.4,lineHeight:1.9}}>BTC · ETH/EVM · TRX · SOL · Scans sync to Supabase · Shared with your team</div>
          </div>}
        </>}
        ...
        {tab==="wallets"&&<>
          ...
          {registryLoading&&<div style={{color:T.dim,fontSize:10,padding:20}}>Loading…</div>}
          <div style={{display:"grid",gridTemplateColumns:"repeat(auto-fill,minmax(300px,1fr))",gap:12}}>
            {registry.map((w,i)=>{const score=w.latest_score??0;const m=RM[rl(score)];const tc=(w.trend||0)>0?T.red:(w.trend||0)<0?T.green:T.mid;
              return <div key={i} onClick={()=>openHistory(w.address)} style={{background:T.panel,border:`1px solid ${selectedWallet===w.address?T.purple:T.border}`,borderRadius:8,padding:16,cursor:"pointer",transition:"border-color 0.15s"}} onMouseEnter={e=>e.currentTarget.style.borderColor=T.purple} onMouseLeave={e=>e.currentTarget.style.borderColor=selectedWallet===w.address?T.purple:T.border}>
                <div style={{display:"flex",justifyContent:"space-between",alignItems:"flex-start",marginBottom:10}}>
                  <div><div style={{fontSize:10,color:T.accent,fontFamily:T.mono,marginBottom:3}}>{w.address?.slice(0,12)}…{w.address?.slice(-8)}</div><div style={{fontSize:8.5,color:T.mid}}>{w.chain} · {w.entity}</div></div>
        ...
        {/* ══ HISTORY ══ */}
        {tab==="history"&&<>
          {!selectedWallet&&<div style={{color:T.dim,fontSize:10,padding:20,textAlign:"center"}}>Select a wallet from REGISTRY to view its scan history.</div>}
          {selectedWallet&&<>
            <div style={{background:T.panel,border:`1px solid ${T.border}`,borderRadius:8,padding:14,marginBottom:14,display:"flex",justifyContent:"space-between",alignItems:"center",flexWrap:"wrap",gap:10}}>
              <div><div style={{fontSize:10,color:T.accent,fontFamily:T.mono,marginBottom:2}}>{selectedWallet}</div><div style={{fontSize:8.5,color:T.mid}}>{walletScans.length} scan{walletScans.length!==1?"s":""} · {walletChanges.length} change events</div></div>
            ...
            {historyTab==="timeline"&&<>
              ...
              {walletScans.length>1&&(()=>{const scores=walletScans.map(s=>s.overall_score);const latest=walletScans[0],oldest=walletScans[walletScans.length-1];const totalDelta=latest.overall_score-oldest.overall_score;
                return <div style={{display:"grid",gridTemplateColumns:"repeat(4,1fr)",gap:12,marginBottom:14}}>
                  {[["LATEST",latest.overall_score,RM[rl(latest.overall_score)].color],["PEAK",Math.max(...scores),RM[rl(Math.max(...scores))].color],["LOWEST",Math.min(...scores),RM[rl(Math.min(...scores))].color],["TOTAL DRIFT",totalDelta>0?`+${totalDelta}`:totalDelta,totalDelta>0?T.red:totalDelta<0?T.green:T.mid]].map(([k,v,c])=>(
            ...
            {historyTab==="changes"&&<div style={{background:T.panel,border:`1px solid ${T.border}`,borderRadius:8,overflow:"hidden"}}>
              ...
              {walletChanges.length===0&&<div style={{padding:20,textAlign:"center",color:T.dim,fontSize:10}}>No changes recorded yet. Changes are detected on every re-scan.</div>}
        ...
        {tab==="log"&&<>
          <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:14}}>
            <span style={{fontSize:9,color:T.mid,letterSpacing:"0.14em"}}>DFSA COMPLIANCE AUDIT LOG — live from Supabase</span>
          ...
          <div style={{background:T.panel,border:`1px solid ${T.border}`,borderRadius:8,overflow:"auto"}}>
            <table style={{width:"100%",borderCollapse:"collapse",fontSize:9.5,fontFamily:T.mono,minWidth:800}}>
